...
def test_grade_from_percent_bands():
    assert academics.grade_from_percent(100) == "O"
    assert academics.grade_from_percent(95) == "A+"   # below 100 -> A+, not O
    ...
    assert academics.grade_from_percent(41) == "P"
...
def test_grade_respects_custom_weightage_bug5():
    ...
    # of 45 is 90% and must be A+, not "F" or "P".
    assert academics.grade_for_total(50, 50) == "O"   # 100%
    assert academics.grade_for_total(45, 50) == "A+"  # 90%
    assert academics.grade_for_total(20, 50) == "P"   # 40%
...
def test_sgpa_credit_weighted():
    # A+ (9) x 4cr and C (5) x 2cr -> (36 + 10) / 6 = 7.667
    value, credits = academics.sgpa([("A+", 4), ("C", 2)])
    assert credits == 6.0
    assert value == Decimal("7.667")


def test_sgpa_perfect_score():
    # O (10) across the board -> 10.0
    value, credits = academics.sgpa([("O", 4), ("O", 3)])
    assert credits == 7.0
    assert value == Decimal("10.000")


def test_sgpa_zero_credits():
    value, credits = academics.sgpa([("O", 0)])
